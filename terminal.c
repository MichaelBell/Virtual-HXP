...
#define FONT_LAST_ASCII (FONT_FIRST_ASCII + FONT_N_CHARS - 1)
...
void terminal_start() {
	...
	for (uint y = 0; y < CHAR_ROWS; ++y) {
		for (uint x = 0; x < CHAR_COLS; ++x) {
			...
			terminal_set_colour(x, y, 0x3f, 0);
		}
	}

	hw_set_bits(&bus_ctrl_hw->priority, BUSCTRL_BUS_PRIORITY_PROC1_BITS);
	multicore_launch_core1(terminal_core1_main);
}

void terminal_puts(int x, int y, char* str) {
	if (x >= CHAR_COLS || y >= CHAR_ROWS)
		return;
  while (*str >= FONT_FIRST_ASCII) {
    terminal_set_char(x++, y, *str++);
    if (x == CHAR_COLS) {
      x = 0;
      if (++y == CHAR_ROWS) y = 0;
    }
